...
└─────────────┘             └──────────────────┘             └─────────────────┘
                                                                      │
                                                    chrome.tabs / scripting / debugger
                                                                      │
                                                             ┌─────────────────┐
                                                             │  Browser Tabs    │
                                                             └─────────────────┘
...
- **Browser Extension** — Chrome/Edge 扩展，优先使用 `chrome.tabs` / `chrome.scripting` 轻量控制标签页，必要时才使用 `chrome.debugger`，**已预构建，直接加载**
...
| 内容提取 | `screenshot`, `get_content`, `get_console_logs`, `wait_for_element` |
...
Arc Tunnel 会尽量先使用不会触发浏览器调试提示的轻量路径：

- 普通截图使用 `chrome.tabs.captureVisibleTab`
- `execute_script`、`get_content`、`wait_for_element` 优先使用 `chrome.scripting`
- cookie/localStorage/sessionStorage 管理不需要 `chrome.debugger`

只有 `snapshot`、`interact`、全页截图、录制回放、部分导航和轻量路径失败后的 fallback 会 attach `chrome.debugger`。这些场景会触发 Chrome/Edge 原生的“正在操作/调试浏览器”提示；Arc Tunnel 会在普通 CDP 操作结束后保留短暂复用窗口，并在空闲约 15 秒后自动 detach。录制期间会保持 debugger 连接，停止录制后再进入自动 detach 流程。
...
get_content({ tabId, mode? })          // mode: 'html' | 'text' | 'structured' | 'markdown'
...
```bash
# 运行测试
cd mcp-server && npm test && cd ..     # 15 tests, 5 suites
cd extension && npx tsc --noEmit && cd ..  # 严格 TypeScript 验证
...
- 默认优先使用轻量浏览器 API，仅在必须使用 CDP 的工具中 attach `chrome.debugger`
- 普通 CDP 操作会在空闲约 15 秒后自动 detach，减少浏览器顶部调试提示的持续显示
...
- **v1.2** — 轻量优先控制路径, 恢复 `get_content` / `wait_for_element`, 普通 CDP 操作空闲自动 detach
